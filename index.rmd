---
title: "Community Building Playbook"
site: bookdown::bookdown_site
documentclass: book
url: https://Drepute.github.io/playbook/
cover-image: "img/cover.png"
description: "This is community building playbook originally authored by @InfiniteGwei from rep3. This book is a public good. We strongly encourage you to read, share and contribute to this resource!"
github-repo: Drepute/playbook
output:
  bookdown::gitbook:
    config:
      toc:
        before: |
          <li><a href="https://Drepute.github.io/playbook/"><center>Community Building Playbook</center></a></li>
        after: |
          <li><a href="https://twitter.com/rep3gg/"><i>Maintained by rep3</i> 😄</a></li>
          <li><a href="https://github.com/rstudio/bookdown"><i>Published using bookdown</i> 📕</a></li>
        collapse: section
        scroll_highlight: true
      toolbar:
        position: null
      edit: https://github.com/Drepute/playbook/edit/main/%s
      download: null
      search:
        engine: lunr
        options: null
      fontsettings:
        theme: dark
        family: sans
        size: 2.5
      sharing:
        twitter: true
        linkedin: true
      info: true
      includes:
        in_header: analytics.html
        after_body: disqus.html
---

# Hey there! {-}

```{css, echo=FALSE}
.book .book-body .page-wrapper .page-inner section.normal h1 {
    font-size: 0;
}
```


![](img/cover.png)

In our quest to help communities maximize their potential, we couldn’t help but try and understand how communities work at a deeper level.

- What dynamics govern them? 
- How does an average community evolve over time? 
- And, most importantly, how does one start a community around an interest or a goal?

To answer these questions, we started putting together this community building playbook as a public good. And we invite you to co-create it with us!

Distilling community building to a formula is no simple task, and it is also not our intention here. Instead, we aim to open a two-way dialogue with the reader, and discuss the general direction we should move in. As a result, this public good may seem incomplete, and it shall always be so.

As community members, we are all well aware of the infinite ways in which communities rise and fall. To add to this complexity, technological developments often disrupt established processes and ground rules. As a result, the community building meta keeps changing.

The rules of building a decentralized, digital community differ from building, say, an irl community. This playbook mainly focusses on the former type of communities, and highlights the concepts and practices that *seem* to work in this domain.

We invite you to share your experiences so we may refine this playbook, and make it a possible reference point for the community builders of tomorrow. 😄
